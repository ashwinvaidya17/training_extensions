...
class OpenVINOClassificationInferencer(BaseInferencer):
    ...
    def __init__(
        ...
        self.model = Model.create_model(
            "ote_classification", model_adapter, self.configuration, preload=True
        )
...
class OpenVINOClassificationTask(
    IDeploymentTask, IInferenceTask, IEvaluationTask, IOptimizationTask
):
    @check_input_parameters_type()
    def __init__(self, task_environment: TaskEnvironment):
        self.task_environment = task_environment
        self.hparams = self.task_environment.get_hyper_parameters(
    ...
    def load_inferencer(self) -> OpenVINOClassificationInferencer:
        return OpenVINOClassificationInferencer(
            self.hparams,
    ...
    def infer(
        ...
        for i, dataset_item in enumerate(dataset, 1):
            ...
            feature_vec_media = TensorEntity(
                name="representation_vector", numpy=repr_vector.reshape(-1)
            )
