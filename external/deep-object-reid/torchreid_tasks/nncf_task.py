...
from ote.api.entities.train_parameters import default_progress_callback
...
class OTEClassificationNNCFTask(OTEClassificationInferenceTask, IOptimizationTask):
    ...
    def __init__(self, task_environment: TaskEnvironment):
        ...
        if not self._cfg.nncf.nncf_config_path:
            self._cfg.nncf.nncf_config_path = os.path.join(
                self._base_dir, "compression_config.json"
            )
        ...
        if task_environment.model.optimization_type == ModelOptimizationType.NNCF:
            ...
            (
                self._compression_ctrl,
                self._model,
                self._nncf_metainfo,
            ) = self._load_nncf_model(task_environment.model)
    ...
    def _set_attributes_by_hyperparams(self):
        ...
        logger.info(
            f"maximal_accuracy_degradation = "
            f"{self._hyperparams.nncf_optimization.maximal_accuracy_degradation}"
        )
        logger.info(
            f"enable_quantization = {self._hyperparams.nncf_optimization.enable_quantization}"
        )
        logger.info(
            f"enable_pruning = {self._hyperparams.nncf_optimization.enable_pruning}"
        )
        self._max_acc_drop = (
            self._hyperparams.nncf_optimization.maximal_accuracy_degradation / 100.0
        )
    ...
    def _load_model(
        ...
        if model is None:
            raise ValueError(
                "No trained model in the project. NNCF require pretrained weights to compress the model"
            )
        ...
        model_data = (
            pretrained_dict
            if pretrained_dict
            else self._load_model_data(model, "weights.pth")
        )
        if is_nncf_state(model_data):
            raise ValueError(
                "Model optimization type is not consistent with the model checkpoint."
            )
    ...
    def _load_nncf_model(self, model: ModelEntity):
        ...
        if not is_nncf_state(model_data):
            raise ValueError(
                "Model optimization type is not consistent with the NNCF model checkpoint."
            )
        model = self._create_model(self._cfg, from_scratch=True)

        compression_ctrl, model, nncf_metainfo = wrap_nncf_model(
            model, self._cfg, checkpoint_dict=model_data
        )
    ...
    def optimize(
        ...
        if self._compression_ctrl:
            raise RuntimeError(
                "The model is already optimized. NNCF requires the original model for optimization."
            )
        if self._cfg.lr_finder.enable:
            raise RuntimeError(
                "LR finder could not be used together with NNCF compression"
            )
        ...
        num_epoch = self._cfg.nncf_config["accuracy_aware_training"]["params"][
        ...
        time_monitor = OptimizationProgressCallback(
            ...
            num_epoch=num_epoch,
            num_train_steps=max(
                1, math.floor(len(train_subset) / self._cfg.train.batch_size)
            ),
            num_val_steps=0,
            num_test_steps=0,
        ...
        self._compression_ctrl, self._model, self._nncf_metainfo = wrap_nncf_model(
            self._model, self._cfg, datamanager_for_init=datamanager
        )

        time_monitor.on_initialization_end()

        self._cfg.train.lr = calculate_lr_for_nncf_training(
            self._cfg, self._initial_lr, False
        )
