...
from ote.api.entities.metadata import FloatMetadata, FloatType
...
from ote.api.entities.model import OptimizationMethod
...
from ote.api.utils.labels_utils import get_empty_label
...
class OTEClassificationInferenceTask(
    ...
    def __init__(self, task_environment: TaskEnvironment):
        ...
        if (
            ...
            self._multihead_class_info = get_multihead_class_info(
    ...
    def _patch_config(self, base_dir: str):
        ...
        self._cfg.lr_finder.enable = (
            self._hyperparams.learning_parameters.enable_lr_finder
        )
        self._cfg.train.early_stopping = (
            self._hyperparams.learning_parameters.enable_early_stopping
        )
    ...
    def infer(
        ...
        for i in range(scores.shape[0]):
            ...
            if self._multilabel:
                item_labels = get_multilabel_predictions(
                    scores[i], self._labels, activate=False
                )
            elif self._hierarchical:
                ...
                item_labels = get_multiclass_predictions(
                    scores[i], self._labels, activate=False
                )
            ...
            feature_vec_media = TensorEntity(
                name="representation_vector", numpy=feature_vecs[i]
            )
            dataset_item.append_metadata_item(
            ...
            if dump_features:
                actmap = get_actmap(
                    saliency_maps[i], (dataset_item.width, dataset_item.height)
                )
        ...
        with tempfile.TemporaryDirectory() as tempdir:
            ...
            try:
                ...
                with force_fp32(self._model):
                    ...
                    self._model.forward = lambda x: self._model.old_forward(
                        x, return_all=True, apply_scale=True
                    )
                    export_onnx(
                        self._model.eval(),
                        self._cfg,
                ...
                bin_file = [
                    f for f in os.listdir(optimized_model_dir) if f.endswith(".bin")
                ][0]
                xml_file = [
                    f for f in os.listdir(optimized_model_dir) if f.endswith(".xml")
                ][0]
