...
from ote.api.entities.datasets import Subset, DatasetEntity
...
DEFAULT_TEMPLATE_DIR = osp.join(
    "configs", "ote_custom_classification", "efficientnet_b0"
)
...
def test_configuration_yaml():
    ...
    configuration_yaml_loaded = create(
        osp.join("torchreid_tasks", "configuration.yaml")
    )
...
def setup_configurable_parameters(template_dir, max_num_epochs=10):
    model_template = parse_model_template(
        osp.join(template_dir, "template_experimental.yaml")
    )
...
def default_task_setup():
    hyper_parameters, model_template = setup_configurable_parameters(
        DEFAULT_TEMPLATE_DIR, max_num_epochs=5
    )
