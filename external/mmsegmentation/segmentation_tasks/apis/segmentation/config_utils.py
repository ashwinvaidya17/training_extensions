...
def rescale_num_iterations(config: Union[Config, ConfigDict], schedule_scale: float):
    ...
    if config.lr_config.policy == "customstep":
        config.lr_config.step = [
            int(schedule_scale * step) for step in config.lr_config.step
        ]
    elif config.lr_config.policy == "customcos":
        config.lr_config.periods = [
            int(schedule_scale * period) for period in config.lr_config.periods
        ]
