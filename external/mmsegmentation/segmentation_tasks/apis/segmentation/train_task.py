...
from ote.api.entities.train_parameters import (
...
class OTESegmentationTrainingTask(OTESegmentationInferenceTask, ITrainingTask):
    ...
    def train(
        ...
        best_checkpoint_path = self._find_best_checkpoint(
            training_config.work_dir, config.evaluation.metric
        )
        ...
        performance = Performance(
            score=ScoreMetric(value=best_score, name="mDice"),
    ...
    def _find_best_checkpoint(work_dir, metric):
        ...
        candidates = [
            f for f in all_files if f.startswith(name_prefix) and f.endswith(".pth")
        ]
