...
from ote.api.entities.annotation import AnnotationSceneEntity, AnnotationSceneKind
...
DEFAULT_TEMPLATE_DIR = osp.join(
    "configs", "custom-object-detection", "gen3_mobilenetV2_ATSS"
)
...
def test_configuration_yaml():
    ...
    configuration_yaml_loaded = create(
        osp.join("detection_tasks", "apis", "detection", "configuration.yaml")
    )
...
class Sample(unittest.TestCase):
    ...
    def test_sample_on_gpu(self):
        output = run(
            "python detection_tasks/sample/sample.py " f"--export {self.template}",
            shell=True,
            check=True,
        )
        assert output.returncode == 0
...
class API(unittest.TestCase):
    ...
    def test_cancel_training_detection(self):
        ...
        self.assertLess(
            time.time() - start_time, 100, "Expected to stop within 100 seconds."
        )
    ...
    def test_training_progress_tracking(self):
        ...
        self.assertTrue(
            np.all(training_progress_curve[1:] >= training_progress_curve[:-1])
        )
    ...
    def test_nncf_optimize_progress_tracking(self):
        ...
        nncf_task.optimize(
            OptimizationType.NNCF, dataset, nncf_model, optimization_parameters
        )
    ...
    def test_inference_progress_tracking(self):
        ...
        self.assertTrue(
            np.all(inference_progress_curve[1:] >= inference_progress_curve[:-1])
        )
    ...
    @staticmethod
    def eval(
        task: OTEDetectionTrainingTask, model: ModelEntity, dataset: DatasetEntity
    ) -> Performance:
        start_time = time.time()
    ...
    def end_to_end(
        ...
        self.assertEqual(
            list(modelinfo.keys()),
            ["model", "config", "confidence_threshold", "VERSION"],
        )
        ...
        self.assertGreater(
            validation_performance.score.value,
        ...
        self.assertNotEqual(
            first_model.get_data("weights.pth"), new_model.get_data("weights.pth")
        )
        ...
        if model_template.entrypoints.nncf:
            if is_nncf_enabled():
                ...
                if nncf_task._hyperparams.nncf_optimization.enable_quantization:
                    ...
                    assert (
                        nncf_model.precision[0] == nncf_task._precision_from_config[0]
                    )
            else:
    ...
    def test_training_gen3_atss(self):
        self.end_to_end(
            osp.join("configs", "custom-object-detection", "gen3_mobilenetV2_ATSS")
        )

    @e2e_pytest_api
    def test_training_gen3_vfnet(self):
        self.end_to_end(
            osp.join("configs", "custom-object-detection", "gen3_resnet50_VFNet"),
            export_perf_delta_tolerance=0.01,
        )
    ...
    def test_training_maskrcnn_resnet50(self):
        self.end_to_end(
            osp.join("configs", "custom-counting-instance-seg", "resnet50_maskrcnn"),
    ...
    def test_training_maskrcnn_efficientnetb2b(self):
        self.end_to_end(
            osp.join(
                "configs", "custom-counting-instance-seg", "efficientnetb2b_maskrcnn"
            ),
            task_type=TaskType.INSTANCE_SEGMENTATION,
        )
