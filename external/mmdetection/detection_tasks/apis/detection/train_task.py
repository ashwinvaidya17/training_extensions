...
from ote.api.entities.train_parameters import TrainParameters, default_progress_callback
    ...
    def train(
        ...
        if hasattr(self._config.model, "bbox_head") and hasattr(
            ...
            if getattr(
                ...
                self._config, self._model = cluster_anchors(
                    self._config, train_dataset, self._model
                )
        ...
        if len(checkpoint_file_path) == 0:
            ...
            logger.warning(
                f"Multiple candidates for the best checkpoint found: {checkpoint_file_path}"
            )
        ...
        if adaptive_threshold:
            ...
            metric = MetricsHelper.compute_f_measure(
                resultset, vary_confidence_threshold=False
            )
        ...
        performance.dashboard_metrics.extend(
            self._generate_training_metrics(learning_curves, val_map)
        )
