...
from ote.api.entities.shapes.polygon import Polygon, Point
...
class LoadAnnotations:
    @check_input_parameters_type()
    def __init__(
        self, with_bbox: bool = True, with_label: bool = True, with_mask: bool = False
    ):
...
class CocoDataset:
    @check_input_parameters_type(
        {"ann_file": JsonFilePathCheck, "data_root": OptionalDirectoryPathCheck}
    )
    def __init__(
        self,
        ann_file: str,
...
@check_input_parameters_type(
    {"ann_file_path": JsonFilePathCheck, "data_root_dir": DirectoryPathCheck}
)
    ...
    for item in coco_dataset:
        ...
        def create_gt_box(x1, y1, x2, y2, label_name):
            return Annotation(
                ...
                labels=[
                    ScoredLabel(
                        label=find_label_by_name(labels_list, label_name, domain)
                    )
                ],
            )
        ...
        if with_mask:
            ...
            for polygon_group in polygons:
                ...
                for polygon in polygon_group:
                    normalized_polygon = [
                        p / divisor[i % 2] for i, p in enumerate(polygon)
                    ]
                    points = [
                        Point(normalized_polygon[i], normalized_polygon[i + 1])
    ...
    for item in dataset:
        for ann in item.get_annotations(include_empty=False):
            has_detection_labels = any(
                label.domain == Domain.DETECTION
                for label in ann.get_labels(include_empty=False)
            )
...
def get_anchor_boxes(wh_stats: List[tuple], group_as: List[int]):
    ...
    kmeans = KMeans(init="k-means++", n_clusters=sum(group_as), random_state=0).fit(
        wh_stats
    )
