...
from ote.api.entities.label_schema import (
    LabelEntity,
    LabelGroup,
    LabelGroupType,
    LabelSchemaEntity,
...
def load_test_dataset(data_type):
    ...
    def gen_image(resolution, shape=None):
        ...
        for s in shape:
            if s == "rectangle":
                draw.rectangle(
                    (h * 0.1, w * 0.1, h * 0.4, w * 0.4),
                    fill=(0, 192, 192),
                    outline=(0, 0, 0),
                )
            if s == "triangle":
                draw.polygon(
                    ((h * 0.5, w * 0.25), (h, w * 0.25), (h * 0.8, w * 0.5)),
                    fill=(255, 255, 0),
                    outline=(0, 0, 0),
                )
            if s == "pieslice":
                draw.pieslice(
                    ((h * 0.1, w * 0.5), (h * 0.5, w * 0.9)),
                    start=50,
                    end=250,
                    fill=(0, 255, 0),
                    outline=(0, 0, 0),
            ...
            if s == "circle":
                draw.ellipse(
                    (h * 0.5, w * 0.5, h * 0.9, w * 0.9), fill="blue", outline="blue"
                )
            if s == "text":
    ...
    datas = [
        ...
        gen_image(
            (32, 32), shape="pieslice+rectangle+triangle"
        ),  # for multilabel (new)
    ...
    def gen_old_new_dataset(multilabel=False, hierarchical=False):
        ...
        for _ in range(new_repeat):
            for idx in new_img_idx:
                if multilabel:
                    new_train.append(
                        get_image(idx, Subset.TRAINING, ignored_labels=ignored_labels)
                    )
                elif hierarchical:
                    new_train.append(
                        get_image(idx, Subset.TRAINING, ignored_labels=ignored_labels)
...
def get_label_schema(labels, multilabel=False, hierarchical=False):
    ...
    if multilabel:
        ...
        single_labels = [
            label for label in labels if label.name in single_label_classes
        ]
        single_label_group = LabelGroup(
            name="labels", labels=single_labels, group_type=LabelGroupType.EXCLUSIVE
        )
        ...
        for label in labels:
            if label.name in multi_label_classes:
                label_schema.add_group(
                    LabelGroup(
                        name=f"{label.name}____{label.name}_group",
                        labels=[label],
        ...
        main_group = LabelGroup(
            name="labels", labels=labels, group_type=LabelGroupType.EXCLUSIVE
        )
...
def main():
    ...
    labels_schema = get_label_schema(
        labels_list, multilabel=args.multilabel, hierarchical=args.hierarchical
    )
