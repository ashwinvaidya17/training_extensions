...
from ote.algorithms.anomaly.adapters.anomalib.data.dataset import (
...
from ote.algorithms.anomaly.tasks import NNCFTask, OpenVINOTask
...
class OteAnomalyTask:
    ...
    def get_dataclass(
        ...
        Type[AnomalySegmentationDataset],
        Type[AnomalyClassificationDataset],
    ]:
    ...
    def create_task_environment(self) -> TaskEnvironment:
        ...
        hyper_parameters = create_hyper_parameters(
    ...
    def create_task(self, task: str) -> Any:
        ...
        if self.model_template.entrypoints is not None:
            ...
            raise ValueError(
                f"Cannot create {task} task. `model_template.entrypoint` does not have {task}"
            )
    ...
    def optimize_nncf(self) -> None:
        ...
        init_model = ModelEntity(
            ...
            configuration=self.task_environment.get_model_configuration(),
            model_adapters={
                "weights.pth": ModelAdapter(self.trained_model.get_data("weights.pth"))
            },
        )
...
def parse_args() -> Namespace:
    ...
    parser.add_argument(
        "--optimization", choices=("none", "pot", "nncf"), default="none"
    )
