...
class TestAnomalyClassification:
    ...
    def test_ote_config(task_path, template_path):
        ...
        ote_config, task_name = get_config_and_task_name(
            f"{task_path}/configs/{template_path}/template.yaml"
        )
    ...
    @TestDataset(
        num_train=200, num_test=10, dataset_path="./datasets/MVTec", use_mvtec=False
    )
    def test_ote_train_export_and_optimize(
        ...
        base_results = self._trainer.infer(
            task=self._trainer.torch_task, output_model=output_model
        )
        self._trainer.evaluate(task=self._trainer.torch_task, result_set=base_results)

        if (
            task_path == "anomaly_classification"
        ):  # skip this check for anomaly segmentation until we switch metrics
            assert base_results.performance.score.value > 0.5

        # Convert the model to OpenVINO
        self._trainer.export()
        openvino_results = self._trainer.infer(
            task=self._trainer.openvino_task, output_model=output_model
        )
        self._trainer.evaluate(
            task=self._trainer.openvino_task, result_set=openvino_results
        )
        ...
        base_nncf_results = self._trainer.infer(
            task=self._trainer.torch_task, output_model=output_model
        )
        self._trainer.evaluate(
            task=self._trainer.torch_task, result_set=base_nncf_results
        )
        if (
            task_path == "anomaly_classification"
        ):  # skip this check for anomaly segmentation until we switch metrics
