...
def _get_dataset_params_from_dataset_definitions(dataset_definitions, dataset_name):
    ...
    print(
        f"training_parameters_fields after make_paths_be_abs: {training_parameters_fields}"
    )
...
def _create_anomaly_dataset_and_labels_schema(
    ...
    category_list = [
        f.path for f in os.scandir(dataset_params.dataset_path) if f.is_dir()
    ]
    items = []
    if "short" in dataset_name:
        ...
        items.extend(
            OteMvtecDataset(
                path=dataset_params.dataset_path, seed=0, task_type=task_type
            ).generate()
        )
    else:
        for category in category_list:
            ...
            items.extend(
                OteMvtecDataset(path=category, seed=0, task_type=task_type).generate()
            )
