...
class AnomalySegmentationTrainingTestParameters(
    DefaultOTETestCreationParametersInterface
):
...
class AnomalySegmentationTestTrainingAction(OTETestTrainingAction):
    ...
    def _run_ote_training(self, data_collector: DataCollector):
        ...
        params = ote_api_configuration_helper_create(
            self.model_template.hyper_parameters.data
        )
        if hasattr(params, "model") and hasattr(
            params.learning_parameters, "early_stopping"
        ):
            if self.num_training_iters != KEEP_CONFIG_FIELD_VALUE:
                params.learning_parameters.early_stopping.patience = int(
        ...
        if self.batch_size != KEEP_CONFIG_FIELD_VALUE:
            ...
            logger.debug(
                f"Set params.learning_parameters.train_batch_size="
                f"{params.learning_parameters.train_batch_size}"
            )
        else:
            logger.debug(
                f"Keep params.learning_parameters.train_batch_size="
                f"{params.learning_parameters.train_batch_size}"
            )
        ...
        self.copy_hyperparams = deepcopy(
            self.task.task_environment.get_hyper_parameters()
        )
