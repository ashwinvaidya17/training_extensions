...
import numpy as np
import ote.algorithms.anomaly.adapters.anomalib.exportable_code
...
from ote.algorithms.anomaly.adapters.anomalib.config import get_anomalib_config
from ote.algorithms.anomaly.adapters.anomalib.logger import get_logger
...
class OpenVINOTask(IInferenceTask, IEvaluationTask, IOptimizationTask, IDeploymentTask):
    ...
    def get_config(self) -> ADDict:
        ...
        ote_config = self.task_environment.get_hyper_parameters()  # type: ignore
        config = get_anomalib_config(task_name=task_name, ote_config=ote_config)
        return ADDict(OmegaConf.to_container(config))

    def infer(
        self, dataset: DatasetEntity, inference_parameters: InferenceParameters
    ) -> DatasetEntity:
        """Perform Inference.
        ...
        if self.task_environment.model is None:
            raise Exception(
                "task_environment.model is None. Cannot access threshold to calculate labels."
            )
        ...
        for idx, dataset_item in enumerate(dataset):
            image_result = self.inferencer.predict(
                dataset_item.numpy, meta_data=meta_data
            )
            ...
            probability = (
                image_result.pred_score if pred_label else 1 - image_result.pred_score
            )
            if self.task_type == TaskType.ANOMALY_CLASSIFICATION:
                label = (
                    self.anomalous_label
                    if image_result.pred_score >= 0.5
                ...
                label = (
                    self.normal_label if len(annotations) == 0 else self.anomalous_label
                )
            ...
            dataset_item.append_labels(
                [ScoredLabel(label=label, probability=float(probability))]
            )
            anomaly_map = anomaly_map_to_color_map(
                image_result.anomaly_map, normalize=False
            )
    ...
    def get_meta_data(self) -> Dict:
        ...
        if self.task_environment.model is None:
            raise ValueError("Model in task is None, cannot get data.")
        ...
        min_value = np.frombuffer(
            self.task_environment.model.get_data("min"), dtype=np.float32
        )
        max_value = np.frombuffer(
            self.task_environment.model.get_data("max"), dtype=np.float32
        )
        meta_data = dict(
    ...
    def _get_optimization_algorithms_configs(self) -> List[ADDict]:
        ...
        hparams = self.task_environment.get_hyper_parameters()  # type: ignore
    ...
    def optimize(
        ...
        data_loader = OTEOpenVINOAnomalyDataloader(
            config=self.config, dataset=dataset, inferencer=self.inferencer
        )
        ...
        engine = IEEngine(
            config=ADDict({"device": "CPU"}), data_loader=data_loader, metric=None
        )
    ...
    def deploy(self, output_model: ModelEntity) -> None:
        ...
        with ZipFile(zip_buffer, "w") as arch:
            ...
            for root, _, files in os.walk(
                os.path.dirname(
                    ote.algorithms.anomaly.adapters.anomalib.exportable_code.__file__
                )
            ):
                for file in files:
