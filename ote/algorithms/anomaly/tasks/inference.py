...
from ote.algorithms.anomaly.adapters.anomalib.callbacks import (
    AnomalyInferenceCallback,
    ProgressCallback,
)
from ote.algorithms.anomaly.adapters.anomalib.config import get_anomalib_config
from ote.algorithms.anomaly.adapters.anomalib.data import OTEAnomalyDataModule
...
class InferenceTask(IInferenceTask, IEvaluationTask, IExportTask, IUnload):
    ...
    def get_config(self) -> Union[DictConfig, ListConfig]:
        ...
        """
        self.hyper_parameters = self.task_environment.get_hyper_parameters()  # type: ignore
        config = get_anomalib_config(
            task_name=self.model_name, ote_config=self.hyper_parameters
        )
    ...
    def cancel_training(self) -> None:
        ...
        cancel_training_file_path = os.path.join(
            self.config.project.path, ".stop_training"
        )
    ...
    def infer(
        ...
        """
        logger.info(
            "Performing inference on the validation set using the base torch model."
        )
        config = self.get_config()
        datamodule = OTEAnomalyDataModule(
            config=config, dataset=dataset, task_type=self.task_type
        )
    ...
    def evaluate(
        ...
        if self.task_type == TaskType.ANOMALY_CLASSIFICATION:
            accuracy = MetricsHelper.compute_accuracy(
                output_resultset
            ).get_performance()
            output_resultset.performance.dashboard_metrics.extend(
                accuracy.dashboard_metrics
            )
    ...
    def export(self, export_type: ExportType, output_model: ModelEntity) -> None:
        ...
        ), f"Incorrect export_type={export_type}"
    ...
    def _set_metadata(self, output_model: ModelEntity):
        output_model.set_data(
            "image_threshold", self.model.image_threshold.value.cpu().numpy().tobytes()
        )
        output_model.set_data(
            "pixel_threshold", self.model.pixel_threshold.value.cpu().numpy().tobytes()
        )
