...
def get_anomalib_config(
    task_name: str, ote_config: ConfigurableParameters
    ...
    """
    config_path = (
        Path(anomalib.__file__).parent / "models" / task_name.lower() / "config.yaml"
    )
    anomalib_config = get_configurable_parameters(
        model_name=task_name.lower(), config_path=config_path
    )
    update_anomalib_config(anomalib_config, ote_config)
    return anomalib_config


def _anomalib_config_mapper(
    anomalib_config: Union[DictConfig, ListConfig], ote_config: ConfigurableParameters
):
    ...
    for name in parameters:
        if name == "train_batch_size":
            anomalib_config.dataset["train_batch_size"] = getattr(
                ote_config, "train_batch_size"
            )
        elif name == "max_epochs":
            anomalib_config.trainer["max_epochs"] = getattr(ote_config, "max_epochs")
        else:
            assert (
                name in anomalib_config.model.keys()
            ), f"Parameter {name} not present in anomalib config."
...
def update_anomalib_config(
    ...
    for param in ote_config.parameters:
        assert (
            param in anomalib_config.keys()
        ), f"Parameter {param} not present in anomalib config."
    ...
    for group in ote_config.groups:
        ...
        if group == "learning_parameters":
            _anomalib_config_mapper(
                anomalib_config, getattr(ote_config, "learning_parameters")
            )
