...
class ProgressCallback(TQDMProgressBar):
    ...
    def __init__(
        self, parameters: Optional[Union[TrainParameters, InferenceParameters]] = None
    ) -> None:
        super().__init__()
    ...
    def _get_progress(self, stage: str = "train") -> float:
        ...
        if stage == "train":
            ...
            self._progress = (
                ...
                / (self.total_predict_batches_current_dataloader + 1e-10)
            ) * 100

        elif stage == "test":
            self._progress = (
                self.test_batch_idx
                / (self.total_test_batches_current_dataloader + 1e-10)
            ) * 100
        else:
            raise ValueError(
                f"Unknown stage {stage}. Available: train, predict and test"
            )
