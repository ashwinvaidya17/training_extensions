...
class AnomalyInferenceCallback(Callback):
    ...
    def __init__(
        self, ote_dataset: DatasetEntity, labels: List[LabelEntity], task_type: TaskType
    ):
    ...
    def on_predict_epoch_end(
        self, _trainer: pl.Trainer, pl_module: AnomalyModule, outputs: List[Any]
    ):
        ...
        for dataset_item, pred_score, pred_label, anomaly_map, pred_mask in zip(
            ...
            dataset_item.append_metadata_item(
                ResultMediaEntity(
                    ...
                    numpy=anomaly_map_to_color_map(
                        anomaly_map.squeeze(), normalize=False
                    ),
                )
            )
            logger.info(
