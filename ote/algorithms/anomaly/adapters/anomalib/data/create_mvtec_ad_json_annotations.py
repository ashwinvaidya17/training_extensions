...
def create_polygons_from_mask(mask_path: str) -> List[List[List[float]]]:
    ...
    polygons = [
        [[point[0][0] / width, point[0][1] / height] for point in polygon]
...
def create_classification_json_items(pd_items: pd.DataFrame) -> Dict[str, Any]:
    ...
    for index, pd_item in pd_items.iterrows():
        json_items["image_path"][str(index)] = pd_item.image_path.replace(
            pd_item.path, ""
        )[1:]
        json_items["label"][str(index)] = pd_item.label
        if pd_item.label != "good":
            json_items["masks"][str(index)] = pd_item.mask_path.replace(
                pd_item.path, ""
            )[1:]
...
def create_detection_json_items(pd_items: pd.DataFrame) -> Dict[str, Any]:
    ...
    for index, pd_item in pd_items.iterrows():
        ...
        json_items["label"][str(index)] = pd_item.label
        if pd_item.label != "good":
            json_items["bboxes"][str(index)] = create_bboxes_from_mask(
                pd_item.mask_path
            )
...
def create_segmentation_json_items(pd_items: pd.DataFrame) -> Dict[str, Any]:
    ...
    for index, pd_item in pd_items.iterrows():
        ...
        json_items["label"][str(index)] = pd_item.label
        if pd_item.label != "good":
            json_items["masks"][str(index)] = create_polygons_from_mask(
                pd_item.mask_path
            )
...
def create_task_annotations(task: str, data_path: str, annotation_path: str) -> None:
    ...
    for split in ["train", "val", "test"]:
        ...
        if task == "classification":
            ...
            raise ValueError(
                f"Unknown task {task}. Available tasks are classification, detection and segmentation."
            )

        df_items = make_mvtec_dataset(
            path=Path(data_path), create_validation_set=True, split=split
        )
...
def create_mvtec_ad_annotations(
    mvtec_data_path: str, mvtec_annotation_path: Optional[str] = None
) -> None:
    ...
    for category in categories:
        ...
        create_mvtec_ad_category_annotations(
            category_data_path, category_annotation_path
        )
...
def get_args() -> Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        required=False,
        help="Path to create OTE CLI annotations.",
    )
...
def main():
    ...
    create_mvtec_ad_annotations(
        mvtec_data_path=args.data_path, mvtec_annotation_path=args.annotation_path
    )
