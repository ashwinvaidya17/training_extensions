...
class OteMvtecDataset:
    ...
    def __init__(
        ...
        self.normal_label = LabelEntity(
            name="Normal", domain=self.label_domain, id=ID(), color=Color(0, 255, 0)
        )
    ...
    def generate(self) -> DatasetEntity:
        ...
        for _, sample in samples.iterrows():
            ...
            # Create annotation
            if (
                self.task_type == TaskType.ANOMALY_CLASSIFICATION
                or sample.label == self.normal_label
            ):
                ...
                self.task_type == TaskType.ANOMALY_SEGMENTATION
                and sample.label == self.abnormal_label
            ):
                mask = (
                    cv2.imread(sample.mask_path, cv2.IMREAD_GRAYSCALE) / 255
                ).astype(np.uint8)
                annotations = create_annotation_from_segmentation_map(
                    mask, np.ones_like(mask), self.label_map
                )
                annotation_scene = AnnotationSceneEntity(
                    annotations=annotations, kind=AnnotationSceneKind.ANNOTATION
                )
            ...
            dataset_item = DatasetItemEntity(
                media=image, annotation_scene=annotation_scene, subset=sample.subset
            )
