...
class OTEAnomalyDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Dict[str, Union[int, Tensor]]:
        ...
        if self.task_type in (
            ...
            if any(
                (
                    isinstance(annotation.shape, Polygon)
                    for annotation in dataset_item.get_annotations()
                )
            ):
                mask = mask_from_dataset_item(
                    dataset_item, dataset_item.get_shapes_labels()
                ).squeeze()
            else:
...
class OTEAnomalyDataModule(LightningDataModule):
        ...
        for subset in [Subset.TRAINING, Subset.VALIDATION, Subset.TESTING]:
            ...
            num_normal = len(
                [
                    item
                    for item in dataset
                    if not item.get_shapes_labels()[0].is_anomalous
                ]
            )
            num_anomalous = len(
                [item for item in dataset if item.get_shapes_labels()[0].is_anomalous]
            )
            logger.info(
    ...
    def val_dataloader(self) -> Union[DataLoader, List[DataLoader]]:
        ...
        if contains_anomalous_images(local_dataset):
            logger.info(
                "Dataset contains polygon annotations. Passing masks to anomalib."
            )
            dataset = OTEAnomalyDataset(self.config, local_dataset, self.task_type)
        else:
            logger.info(
                "Dataset does not contain polygon annotations. Not passing masks to anomalib."
            )
            dataset = OTEAnomalyDataset(
                self.config, global_dataset, TaskType.ANOMALY_CLASSIFICATION
            )
        return DataLoader(
            dataset,
            shuffle=False,
    ...
    def predict_dataloader(self) -> Union[DataLoader, List[DataLoader]]:
        ...
        """
        dataset = OTEAnomalyDataset(
            self.config, self.predict_ote_dataset, self.task_type
        )
        return DataLoader(
            dataset,
            shuffle=False,
